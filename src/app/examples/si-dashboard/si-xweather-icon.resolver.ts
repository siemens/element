 ...
 * this example app under `assets/weather-icons/`.
 *
 * This resolver lives in the examples folder on purpose: the Element library
...
/**
 * Maps {@link SiWeatherCondition} tokens to Xweather base filenames (without
 * the optional modifier letters, `n` night suffix, or `.png` extension).
 *
 * The widget passes through any condition token from the three vocabulary
 * tiers; tokens that this resolver does not recognise yield `null` and no
 * illustration is rendered.
 */
...
/**
 * Known icon filenames bundled in `assets/weather-icons/` (without the
 * `.svg` extension). Used to fall back to a less-decorated variant when a
 * specific modifier combination has no asset, rather than emitting a 404.
 */
...
@Injectable()
export class SiXweatherIconResolver extends SiWeatherIconResolver {
  override resolve(icon: SiWeatherIcon): SiWeatherIconResolution | null {
    ...
    // Step 1: pick a working base, possibly switching for `freezing` when a
    // dedicated freezing-variant base exists.
    ...
    // Step 2: append at most one weather-letter (`t` thunder > `s` snow >
    // `r` rain) on bases that have those composite variants. `freezing` + `snow`
    // produces the combined `sf` suffix on `mcloudy` / `pcloudy`.
    let suffix = '';
    ...
    // Step 3: append `w` for windy, then `n` for night. Each suffix is only
    // applied if the resulting filename exists in the bundled icon set; this
    // lets toggles degrade gracefully instead of yielding 404s.
  ...
  /**
   * Returns the candidate filenames to try, in priority order. Each successive
   * candidate strips one decoration so the resolver can fall back to a less
   * specific asset when the fully decorated one is missing.
   */
