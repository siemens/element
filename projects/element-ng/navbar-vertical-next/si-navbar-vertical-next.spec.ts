...
describe('SiNavbarVerticalNext', () => {
  ...
  describe('without SiUIStateService', () => {
    ...
    describe('flat group (mobile)', () => {
      ...
      it('should retain inline group behavior on mobile when alwaysFlyout is disabled', async () => {
        component.showDeclarativeFlyoutGroup.set(true);
        await fixture.whenStable();
        ...
        await expect.element(page.getByRole('button', { name: 'Go back' })).not.toBeInTheDocument();
        expect(getNavbarHost()).not.toHaveClass('nav-flat-group-open');
      });
