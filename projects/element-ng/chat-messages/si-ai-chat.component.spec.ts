...
import { ChatMessage, MessageAction, SiAiChatComponent } from '@siemens/element-ng/chat-messages';

describe('SiAiChatComponent', () => {
  let fixture: ComponentFixture<SiAiChatComponent>;
  let debugElement: DebugElement;
  let component: SiAiChatComponent;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [SiAiChatComponent],
      providers: [provideZonelessChangeDetection()]
    }).compileComponents();

    fixture = TestBed.createComponent(SiAiChatComponent);
  ...
  it('should show output instead of inputs in auto mode', () => {
    ...
    expect(markdown.componentInstance.markdown()).toBe('Found results');
  });

  it('should render a single string input directly', () => {
    fixture.componentRef.setInput('show', 'inputs');
    fixture.componentRef.setInput('messages', [
      { type: 'activity', name: 'Editing', input: 'Updated content' }
    ] satisfies ChatMessage[]);
    fixture.detectChanges();
  ...
  it('should collapse details after a newer activity', () => {
    ...
    const markdown = debugElement.queryAll(By.css('si-activity-message si-markdown'));
    expect(activities[0].componentInstance.expanded()).toBe(false);
    expect(activities[1].componentInstance.expanded()).toBe(true);
    expect(debugElement.query(By.css('si-activity-message-part'))).toBeNull();
    expect(markdown).toHaveLength(1);
    expect(markdown[0].componentInstance.markdown()).toBe('Second output');
  });
  ...
  it('should compute inputInterruptible correctly', () => {
    ...
    expect(component.inputInterruptible()).toBe(true);
    ...
    expect(component.inputInterruptible()).toBe(false);
  });
  ...
  it('should compute inputSending correctly', () => {
    ...
    expect(component.inputSending()).toBe(true);
    ...
    expect(component.inputSending()).toBe(false);
  });
  ...
  it('should handle messages with more than 3 actions', async () => {
    const actions: MessageAction[] = [
