...
import { MenuItem, MenuItemAction } from '@siemens/element-ng/menu';
...
const EMPTY_MESSAGE_ACTIONS: MessageActions = { primary: [], secondary: [] };
...
@Component({
  ...
  templateUrl: './si-ai-chat.component.html',
  providers: [{ provide: SI_CHAT_INPUT_PARENT, useExisting: SiAiChatComponent }],
  host: {
    class: 'd-flex si-layout-inner flex-grow-1 flex-column h-100 w-100'
  }
})
export class SiAiChatComponent implements SiChatInputParent {
  ...
  protected readonly messageActions = computed(() => {
    const actions = new WeakMap<ChatMessage, MessageActions>();
    ...
    for (const message of this.displayMessages()) {
      ...
      actions.set(message, {
        primary: messageActions.slice(0, 3),
        secondary: messageActions.slice(3).map(
          action =>
            ({
              ...action,
              action: action.action as unknown as MenuItemAction['action'],
              type: 'action'
            }) as MenuItem
        )
      });
  ...
  protected getMessageActions(message: ChatMessage): MessageActions {
    return this.messageActions().get(message) ?? EMPTY_MESSAGE_ACTIONS;
  }
  ...
  protected shouldAutoExpandActivity(
    ...
    const directDetail = this.getDirectActivityDetailType(message);
    const directAutoExpand =
      directDetail === 'input' && typeof message.autoExpandInputs === 'boolean'
        ? message.autoExpandInputs
        : directDetail === 'output'
          ? message.autoExpandOutputs
          : undefined;
    return (
      active &&
      (directAutoExpand ??
        ((message.autoExpand ?? inheritedAutoExpand) || this.hasExplicitAutoExpand(message)))
    );
  }
  ...
  protected getDirectActivityDetail(message: ActivityChatMessage): string | undefined {
    const detail = this.getDirectActivityDetailType(message);
    return detail === 'input'
  ...
  private getDirectActivityDetailType(
    message: ActivityChatMessage
  ): 'input' | 'output' | undefined {
    if (message.content || message.messages?.length) {
      return undefined;
    }
    const showInputs = this.shouldShowActivityInputs(message);
    const showOutput = this.shouldShowActivityOutput(message);
    if (showInputs && !showOutput && typeof message.input === 'string') {
      return 'input';
    }
    if (showOutput && !showInputs) {
      return 'output';
    }
    return undefined;
  }
