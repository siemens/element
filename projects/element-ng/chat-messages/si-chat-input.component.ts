...
export class SiChatInputComponent implements AfterViewInit {
  ...
  private registeredSending: Signal<boolean> | undefined;
  private registeredInterruptible: Signal<boolean> | undefined;

  constructor() {
    effect(() => {
      if (this.registeredSending) {
        const sendingValue = this.registeredSending();
        this.sending.set(sendingValue);
      }
    });

    effect(() => {
      if (this.registeredInterruptible) {
        const interruptibleValue = this.registeredInterruptible();
        this.interruptible.set(interruptibleValue);
      }
    });
  }
  ...
  public registerStates(sending: Signal<boolean>, interruptible: Signal<boolean>): void {
    this.registeredSending = sending;
    this.registeredInterruptible = interruptible;
  }
