...
@Directive({
  ...
  host: {
    ...
    '[attr.aria-rowcount]': 'computedRowCount()',
    '[attr.aria-colcount]': 'computedColCount()',
...
export class SiGridDirective {
  ...
  /**
   * Computed row count for aria-rowcount.
   */
  protected readonly computedRowCount = computed(() => this.derivedRows().length);

  /**
   * Computed column count for aria-colcount.
   */
  protected readonly computedColCount = computed(() =>
    Math.max(...this.derivedRows().map(row => row.cells.length), 0)
  );
  ...
  constructor() {
    ...
    // Update cell aria-rowindex and aria-colindex in flat mode
    effect(() => {
      const rows = this.derivedRows();
      rows.forEach((row, rowIdx) => {
        row.cells.forEach((cell, colIdx) => {
          // Only set if not explicitly provided
          if (cell.ariaRowIndex() === undefined) {
            cell.ariaRowIndex.set(rowIdx + 1);
          }
          if (cell.ariaColIndex() === undefined) {
            cell.ariaColIndex.set(colIdx + 1);
          }
        });
      });
    });
