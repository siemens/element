...
export class SiWizardComponent {
  ...
  protected readonly visibleSteps = linkedSignal(() => this.calculateNumberOfVisibleSteps());
  ...
  protected updateNumberOfVisibleSteps(): void {
    const newVisibleSteps = this.calculateNumberOfVisibleSteps();
    if (newVisibleSteps !== this.visibleSteps()) {
      this.visibleSteps.set(newVisibleSteps);
    }
  }

  private calculateNumberOfVisibleSteps(): number {
