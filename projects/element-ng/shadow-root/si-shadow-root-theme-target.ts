...
import { SI_THEME_DOM_TARGET, SiThemeDomTarget, SiThemeService } from '@siemens/element-ng/theme';

const createShadowRootThemeTarget = (): SiThemeDomTarget => {
...
/**
 * Provides the shadow root specific theme service with a dedicated theme target
 * for components using Shadow DOM encapsulation.
 * Use this when the component defines and loads the Element theme in its own shadow root instead
 * of reusing the theme defined on `:root`.
 */
export const provideShadowRootThemeTarget = (): Provider => {
  return [
    SiThemeService,
    {
      provide: SI_THEME_DOM_TARGET,
      useFactory: createShadowRootThemeTarget
    }
  ];
};
