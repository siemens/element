...
export class SiChatContainerComponent implements OnDestroy, AfterContentInit {
  ...
  /**
   * Cache for message actions to prevent object recreation on every change detection cycle.
   * Without caching, the menu factory's `@for` loop would recreate all DOM nodes when the menu opens,
   * causing the menu to close immediately and triggering NG0956 warnings.
   */
  private messageActionsCache = new WeakMap<
    ChatMessage,
    { primary: MessageAction[]; secondary: MenuItemAction[] }
  >();
   ...
   * @defaultValue []
   */
  readonly inputActions = input<MessageAction[]>([]);
  ...
  protected readonly inputSecondaryActions = computed(() => {
    ...
    return actions.slice(2 - hasAttachmentAction).map(
      action =>
        ({
          ...action,
          action: action.action as unknown as (actionParam: any, source: MenuItemAction) => void,
          type: 'action'
        }) as MenuItemAction
    );
  });
  ...
  /**
   * Returns stable references to message actions by caching them per message.
   * This prevents recreating action objects on every change detection cycle,
   * which would cause menu dropdowns to close immediately after opening.
   */
  private getMessageActions(message: ChatMessage): {
    primary: MessageAction[];
    secondary: MenuItemAction[];
  } {
    if (message.type === 'tool' || !('actions' in message) || !message.actions) {
    ...
    const actions = message.actions as MessageAction[];
    let primary: MessageAction[];
    let secondary: MenuItemAction[];
