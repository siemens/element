...
import { MapColorPalette } from './model';
...
/**
 * Creates a configurable signal that returns a StyleSpecification based on the current theme.
...
export const observeStyleJson = (key: string): Signal<StyleSpecification> => {
  ...
  return computed(() => {
    ...
    return styleJson(key, isDark());
  });
};

/**
 * Get the map color palette based on the current Element theme.
 */
...
const styleJson = (key: string, dark: boolean): StyleSpecification => {
