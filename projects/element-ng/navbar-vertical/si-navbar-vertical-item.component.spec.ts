...
describe('SiNavbarVerticalItemComponent', () => {
  ...
  describe('showSubtleBadgeCollapsed behavior', () => {
    ...
    it('should add class when true', () => {
      ...
      expect(linkElement.classList.contains('show-subtle-badge-collapsed')).toBe(true);
    });
    ...
    it('should not add class when false', () => {
      badgeTestComponent.item.set({
        ...
        badge: 5,
        badgeColor: 'default',
        showSubtleBadgeCollapsed: false
      });
      badgeTestFixture.detectChanges();

      const linkElement = badgeTestFixture.nativeElement.querySelector('a.navbar-vertical-item');
      expect(linkElement.classList.contains('show-subtle-badge-collapsed')).toBe(false);
    });
