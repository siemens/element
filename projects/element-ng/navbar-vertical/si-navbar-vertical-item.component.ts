...
@Component({
  ...
  host: {
    ...
    '[class.active]': 'active',
    '[class.show-subtle-badge-collapsed]': 'keepSubtleBadge()'
  }
})
export class SiNavbarVerticalItemComponent implements OnInit {
  ...
  /**
   * Shows subtle badges (non-emphasis) in collapsed state
   */
  protected readonly keepSubtleBadge = computed(() => {
    return !!(this.item() as NavbarVerticalItemInteractive).showSubtleBadgeCollapsed;
  });
