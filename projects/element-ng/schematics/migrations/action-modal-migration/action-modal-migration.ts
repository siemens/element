...
const createActionDialogMethodCallTransformation = (
  ...
  const methodName = expression.name.text as LegacyMethodName;
  ...
  const newNode = ts.factory.createCallExpression(
    ts.factory.createPropertyAccessExpression(
      ...
      ts.factory.createIdentifier('showActionDialog')
    ),
    undefined,
    [ts.factory.createObjectLiteralExpression(dialogPropertiesArray)]
  );
...
const generateDialogProperties = (
  ...
  const dialogProperties: ts.PropertyAssignment[] = [
    ts.factory.createPropertyAssignment(
      ts.factory.createIdentifier('type'),
      ts.factory.createStringLiteral(methodConfig.type, true)
    )
  ];
  ...
  nodeArguments.forEach((arg, index) => {
    ...
    if (paramName) {
      dialogProperties.push(
        ts.factory.createPropertyAssignment(ts.factory.createIdentifier(paramName), arg)
      );
    }
  });
