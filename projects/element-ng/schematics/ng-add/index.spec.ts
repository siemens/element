...
describe('ng-add schematic', () => {
  ...
  it('should add @simpl/brand version that matches the root package.json', async () => {
    const expectedVersion = rootPackageJson.optionalDependencies['@simpl/brand'];

    const tree = await runner.runSchematic('ng-add', { path: '/' }, appTree);

    const simplBrand = getPackageJsonDependency(tree, '@simpl/brand');
    expect(simplBrand?.version).toBe(expectedVersion);
  });

  it('should add @siemens/element-icons version that matches the root package.json', async () => {
    const expectedVersion = rootPackageJson.dependencies['@siemens/element-icons'];

    const tree = await runner.runSchematic('ng-add', { path: '/' }, appTree);

    const elementIcons = getPackageJsonDependency(tree, '@siemens/element-icons');
    expect(elementIcons?.version).toBe(expectedVersion);
  });
