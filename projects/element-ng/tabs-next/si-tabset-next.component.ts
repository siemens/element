...
export class SiTabsetNextComponent implements AfterViewInit {
  ...
  readonly tabContainer = viewChild.required<ElementRef<HTMLDivElement>>('tabContainer');
  readonly tabContentWrapper = viewChild.required<ElementRef<HTMLDivElement>>('tabContentWrapper');
  protected readonly menu = viewChild('menu', { read: CdkMenu });

  protected readonly initialTabIndex = signal(0);
  protected readonly itemsOverflowing = signal(false);

  private readonly tabButtons = computed(() => {
    return this.tabPanels().map(tab => tab.tabButton);
  });
  private readonly cachedScrollDiff = signal(0);

  private readonly tabButtonsWidths = computed(() => {
    return this.tabButtons().map(tabButton => {
      return {
        index: tabButton.nativeElement.clientWidth
      };
    });
  });
  ...
  tabsetId = `${SiTabsetNextComponent.tabCounter++}`;
  ...
  protected mouseScroll(event: WheelEvent): void {
    if (
      this.visibleTabIndexes().length === this.tabPanels().length ||
      !this.visibleTabIndexes().length
    ) {
      return;
    }
    const visibleItemIndex = this.visibleTabIndexes().at(0) ?? 0;

    this.visibleTabIndexes.set([]);
    setTimeout(() =>
      this.tabButtons()
        .at(
          event.deltaX < 0
            ? visibleItemIndex - 1 < 0
              ? 0
              : visibleItemIndex - 1
            : visibleItemIndex + 1
        )
    ...
    if (startIndex < this.tabButtons().length) {
      if (!this.visibleTabIndexes().includes(startIndex) || forceUpdate) {
        ...
        const calculateConsumedWidth = (i: number): boolean => {
          consumedWidth += this.tabButtonsWidths()[i].index;
          if (
