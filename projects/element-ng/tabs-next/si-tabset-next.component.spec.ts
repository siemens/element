...
describe('SiTabsetNext', () => {
  ...
  it('should hide tabs which are not in the view and show menu button', fakeAsync(async () => {
    ...
    testComponent.wrapperWidth.set(300);
    fixture.detectChanges();
    expect(await (await tabsetHarness.getTabItemButtonAt(3)).hasClass('hidden')).toBe(true);
    ...
    expect(await (await tabsetHarness.getTabItemButtonAt(3)).hasClass('hidden')).toBe(false);
  }));
  ...
  it('should bring tab into visibile area if not visible on selection from menu', fakeAsync(async () => {
    ...
    detectSizeChange();
    fixture.detectChanges();
    expect(await (await tabsetHarness.getTabItemButtonAt(4)).hasClass('hidden')).toBe(true);
    ...
    expect(await (await tabsetHarness.getTabItemButtonAt(0)).hasClass('hidden')).toBe(true);
  }));
  ...
  it('should delete tab on close and recaculate visisble tabs', async () => {
    ...
    expect(await (await tabsetHarness.getTabItemButtonAt(2)).hasClass('hidden')).toBe(true);
    expect(await tabsetHarness.getOptionsMenuButton()).toBeDefined();
    ...
    expect(await (await tabsetHarness.getTabItemButtonAt(1)).hasClass('hidden')).toBe(false);
  });
});
