...
import { injectMarkdownRenderer, type MarkdownRenderer } from './markdown-renderer';
...
describe('SiMarkdownRendererComponent', () => {
  ...
  let renderer: WritableSignal<MarkdownRenderer | undefined>;

  describe('with renderer', () => {
    beforeEach(() => {
      text = signal('');
      renderer = signal<MarkdownRenderer | undefined>(
        TestBed.runInInjectionContext(() => injectMarkdownRenderer())
      );
      fixture = TestBed.createComponent(TestComponent, {
        bindings: [inputBinding('text', text), inputBinding('renderer', renderer)]
      });
...
| *Line breaks* | Line 1<br>Line 2<br>Line 3 |`;
    ...
    it('should not render LaTeX when math extensions are not provided', async () => {
      text.set('The formula is $E = mc^2$ in physics.');
      await fixture.whenStable();

      const markdownDiv = hostElement.firstElementChild!;
      expect(markdownDiv.textContent).toContain('$E = mc^2$');
      expect(markdownDiv.querySelector('.katex')).toBeNull();
    });
  });

  describe('with LaTeX support', () => {
    beforeEach(() => {
      text = signal('');
      renderer = signal<MarkdownRenderer | undefined>(
    ...
    it('should render LaTeX inline math expressions', async () => {
      text.set('The formula is $E = mc^2$ in physics.');
      await fixture.whenStable();

      const markdownDiv = hostElement.firstElementChild!;
      expect(markdownDiv.querySelector('.math.math-inline')).toBeTruthy();
      expect(markdownDiv.querySelector('.katex')).toBeTruthy();
    });

    it('should render LaTeX display math expressions', async () => {
      text.set('$$\\int_{-\\infty}^{\\infty} e^{-x^2} dx$$');
      await fixture.whenStable();

      const markdownDiv = hostElement.firstElementChild!;
      expect(markdownDiv.innerHTML).toContain('latex-display-wrapper');
      expect(markdownDiv.querySelector('.math.math-display')).toBeTruthy();
    });

    it('should not treat dollar signs in code blocks as LaTeX', async () => {
      text.set('Formula: $x^2$\n\n```javascript\nconst price = "$100";\n```');
      await fixture.whenStable();
  ...
  describe('without renderer', () => {
    ...
    it('should display raw text when no renderer is provided', async () => {
      text.set('This is **bold** text');
      await fixture.whenStable();

      expect(hostElement.textContent).toBe('This is **bold** text');
      expect(hostElement.querySelector('strong')).toBeNull();
    });

    it('should display empty content for undefined input', async () => {
      text.set(null);
      await fixture.whenStable();
