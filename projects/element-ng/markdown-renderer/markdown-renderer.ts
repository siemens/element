      ...
      const translatedLabel =
        options?.copyCodeButton && options?.translateSync
          ...
          : options?.copyCodeButton
            ...
            : undefined;
      const buttonLabel = translatedLabel?.replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const codeCopyButton = buttonLabel
        ? `<button type="button" class="btn btn-circle btn-sm btn-tertiary element-copy copy-code-btn" data-code-id="${codeId}" aria-label="${buttonLabel}"></button><code ${codeAttributes} id="${codeId}">${escapedCode}`
        : '';
