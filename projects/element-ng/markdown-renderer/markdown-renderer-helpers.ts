...
export const sanitizeHtmlWithStyles = (html: string, sanitizer: DomSanitizer): string | null => {
  ...
  const svgId = `SVG-PH-${Math.random().toString(36).substring(2, 15)}`;
  ...
  processed = processed.replace(/<svg\b[\s\S]*?<\/svg>/gi, svg => {
    if (!isSafeKatexSvg(svg)) {
      return '';
    }
    const placeholder = `${svgId}-${svgMap.size}`;
    svgMap.set(placeholder, svg);
    return placeholder;
  });
  ...
  svgMap.forEach((svg, placeholder) => {
    restored = restored.replace(placeholder, svg);
  });

  return restored;
};

const isSafeKatexSvg = (svg: string): boolean => {
  if (/<\/?(?:script|foreignobject|iframe|object|embed)\b/i.test(svg)) {
    return false;
  }

  if (/\s(?:on\w+|href|xlink:href)\s*=/i.test(svg)) {
    return false;
  }

  return /^<svg\b[^>]*>\s*<path\b[^>]*(?:><\/path>|\/>)\s*<\/svg>$/i.test(svg);
};
