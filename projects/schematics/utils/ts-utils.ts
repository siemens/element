...
export const parseTsconfigFile = (
  ...
  const { config, error } = ts.readConfigFile(tsconfigPath, tree.readText.bind(tree));
  if (error) {
  ...
  const parseConfigHost: ts.ParseConfigHost = {
    ...
    readDirectory: (path, _extensions, _excludes, _includes, _depth) => visitDirectory(tree, path),
...
export const getTemplate = (tree: Tree, decorator: ts.Decorator): AngularTemplate | undefined => {
  ...
  if (metadata.template) {
    return {
      filePath: filePath,
      offset: metadata.template.offset,
...
const visitDirectory = (tree: Tree, dirPath: string): string[] => {
  ...
  entries.subdirs.forEach(subdirname => {
    const newFiles = visitDirectory(tree, `${dirPath}/${subdirname}`);
    files.push(...newFiles);
  });
