...
import { AppXweatherIconResolver } from './app-xweather-icon.resolver';
import {
...
@Component({
  ...
  template: `
    ...
    />
  `,
  providers: [{ provide: SiWeatherIconResolver, useClass: AppXweatherIconResolver }],
    ...
    const data: SiWeatherWidgetData = {
      current: {
        ...
        illustration: {
          ...
          daytime: p.daytime,
          modifiers: p.modifiers?.length ? p.modifiers : undefined
        },
