...
describe('SiNChartGaugeComponent', () => {
  ...
  const defaultSeries: GaugeSeries[] = [
    { value: 30, colorToken: 'si-sys-color-data-categorical-7', name: 'Series A' },
    { value: 20, colorToken: 'si-sys-color-data-categorical-4', name: 'Series B' }
  ];
  ...
  it('should render an optional marker on top of the data arc', async () => {
    ...
    expect(marker?.querySelector('rect')).toHaveAttribute('x', '-1');
    expect(marker?.querySelector('rect')).toHaveAttribute('y', '-3');
    expect(marker?.querySelector('rect')).toHaveAttribute('width', '1');
    expect(marker?.querySelector('rect')).toHaveAttribute('height', '6');
    expect(marker?.querySelector('rect')).toHaveAttribute('rx', '0.25');
  ...
  describe('sum mode', () => {
    ...
    it('should handle positive and negative series values', async () => {
      host.series.set([
        { value: 40, colorToken: 'si-sys-color-data-categorical-7', name: 'Positive' },
        { value: -20, colorToken: 'si-sys-color-data-categorical-1', name: 'Negative' }
      ]);
      await fixture.whenStable();
    ...
    it('should apply data-id attribute when provided', async () => {
      host.series.set([
        { value: 30, colorToken: 'si-sys-color-data-categorical-7', name: 'A', id: 'gauge-a' },
        { value: 20, colorToken: 'si-sys-color-data-categorical-4', name: 'B', id: 'gauge-b' }
      ]);
      await fixture.whenStable();
    ...
    it('should not render data arc for series with value 0', async () => {
      host.series.set([{ value: 0, colorToken: 'si-sys-color-data-categorical-7', name: 'Zero' }]);
      await fixture.whenStable();
  ...
  describe('single mode', () => {
    const segments: GaugeSegment[] = [
      { colorToken: 'si-sys-color-data-categorical-1', endValue: 0 },
      { colorToken: 'si-sys-color-data-categorical-5', endValue: 50 },
      { colorToken: 'si-sys-color-data-categorical-7', endValue: 100 }
    ];
    ...
    it('should render a single data arc', async () => {
      host.series.set([
        { value: 60, colorToken: 'si-sys-color-data-categorical-3', name: 'Value' }
      ]);
      await fixture.whenStable();
  ...
  describe('legend', () => {
    ...
    it('should display description in legend when provided', async () => {
      host.series.set([
        {
          value: 30,
          colorToken: 'si-sys-color-data-categorical-7',
          name: 'Series A',
    ...
    it('should apply data-id on legend rows when provided', async () => {
      host.series.set([
        { value: 30, colorToken: 'si-sys-color-data-categorical-7', name: 'A', id: 'row-a' }
      ]);
      await fixture.whenStable();
  ...
  describe('formatters', () => {
    it('should use custom valueFormatter', async () => {
      host.series.set([
        { value: 42.567, colorToken: 'si-sys-color-data-categorical-7', name: 'A' }
      ]);
    ...
    it('should respect minNumberOfDecimals and maxNumberOfDecimals', async () => {
      host.series.set([{ value: 5, colorToken: 'si-sys-color-data-categorical-7', name: 'A' }]);
  ...
  describe('highlight', () => {
    it('should toggle highlight on data arc mouseenter/mouseleave', async () => {
      host.series.set([{ value: 50, colorToken: 'si-sys-color-data-categorical-7', name: 'A' }]);
      await fixture.whenStable();
