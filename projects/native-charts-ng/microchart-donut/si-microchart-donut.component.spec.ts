...
describe('SiMicrochartDonutComponent', () => {
  ...
  const multiSeries: MicrochartDonutSeries[] = [
    { valuePercent: 30, colorToken: 'si-sys-data-categorical-1', id: 'series-a' },
    { valuePercent: 50, colorToken: 'si-sys-data-categorical-5', id: 'series-b' }
  ];
  ...
  beforeEach(() => {
    series.set([{ valuePercent: 40, colorToken: 'si-sys-data-categorical-4' }]);
    radius.set(7.5);
  ...
  it('should apply correct stroke color from colorToken', async () => {
    ...
    expect(paths[0]).toHaveStyle({ stroke: 'var(--si-sys-data-categorical-1)' });
    expect(paths[1]).toHaveStyle({ stroke: 'var(--si-sys-data-categorical-5)' });
  });
  ...
  it('should skip series with 0 valuePercent', async () => {
    series.set([
      { valuePercent: 0, colorToken: 'si-sys-data-categorical-1' },
  ...
  it('should skip series with negative valuePercent', async () => {
    series.set([{ valuePercent: -10, colorToken: 'si-sys-data-categorical-1' }]);
    await fixture.whenStable();
