...
describe('SiMicrochartLineComponent', () => {
  ...
  beforeEach(() => {
    series.set({ values: [2, 4, 6, 8, 10], colorToken: 'si-sys-color-data-categorical-10' });
    width.set(64);
    height.set(24);
  ...
  it('should render line path with correct stroke color', async () => {
    ...
    expect(path!).toHaveAttribute('stroke', 'var(--si-sys-color-data-categorical-10)');
  });
  ...
  it('should return empty path for fewer than 2 values', async () => {
    series.set({ values: [5], colorToken: 'si-sys-color-data-categorical-10' });
    await fixture.whenStable();
  ...
  it('should use series colorToken for markers by default', async () => {
    ...
    for (const marker of getMarkers()) {
      expect(marker).toHaveAttribute('fill', 'var(--si-sys-color-data-categorical-10)');
    }
  });

  it('should use markerColor for markers when provided', async () => {
    showMarkers.set(true);
    markerColor.set('si-sys-color-data-categorical-2');
    await fixture.whenStable();

    for (const marker of getMarkers()) {
      expect(marker).toHaveAttribute('fill', 'var(--si-sys-color-data-categorical-2)');
    }
  });
