...
describe('SiMicrochartBarComponent', () => {
  ...
  beforeEach(() => {
    series.set({ values: [2, 4, 6, 8, 10], colorToken: 'si-sys-color-data-categorical-7' });
    width.set(64);
    height.set(24);
  ...
  it('should apply color token to bar backgrounds', async () => {
    ...
    for (const bar of getBars()) {
      expect(bar).toHaveStyle({ backgroundColor: 'var(--si-sys-color-data-categorical-7)' });
    }
  });
  ...
  it('should render all-negative values with bars growing downward from zero line', async () => {
    series.set({ values: [-3, -6, -9], colorToken: 'si-sys-color-data-categorical-5' });
    await fixture.whenStable();
  ...
  it('should render mixed positive/negative values with zero line', async () => {
    series.set({ values: [10, -10], colorToken: 'si-sys-color-data-categorical-3' });
    await fixture.whenStable();
  ...
  it('should use negativeColorToken for negative values', async () => {
    series.set({
      values: [5, -5],
      colorToken: 'si-sys-color-data-categorical-7',
      negativeColorToken: 'si-sys-color-data-categorical-1'
    });
    await fixture.whenStable();

    const bars = getBars();
    expect(bars[0]).toHaveStyle({ backgroundColor: 'var(--si-sys-color-data-categorical-7)' });
    expect(bars[1]).toHaveStyle({ backgroundColor: 'var(--si-sys-color-data-categorical-1)' });
  });

  it('should use main colorToken for negative values when no negativeColorToken', async () => {
    series.set({ values: [5, -5], colorToken: 'si-sys-color-data-categorical-7' });
    await fixture.whenStable();

    const bars = getBars();
    expect(bars[1]).toHaveStyle({ backgroundColor: 'var(--si-sys-color-data-categorical-7)' });
  });
});
