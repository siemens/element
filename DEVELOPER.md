...
- [Python](https://www.python.org/) Version as specified in [pyproject.toml](pyproject.toml)
...
Then you can example application with the corresponding npm script:
...
```shell
# On the extra parameter can be omitted
npm run start --  --host 127.0.0.1

# Run the E2E tests on another terminal
./e2e-local.sh

# To update the e2e tests append: update or update-all
./e2e-local.sh update
...
PLAYWRIGHT_staticTest=buttons/buttons ./e2e-local.sh run static
```
