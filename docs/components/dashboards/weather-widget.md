...
- Provide a `SiWeatherIconResolver` at application root. Without one, the
  widget cannot render illustrations from condition tokens.
...
> 4. Condition label, 5. Min/Max range, 6. Metrics list, 7. Forecast list
...
The library ships only the abstract token; applications provide a concrete
resolver that maps the widget's condition vocabulary to image assets. See
[Weather condition tokens](weather-condition-tokens.md) for the token
