...
- On a [table](../lists-tables-trees/overview.md) or [list](../lists-tables-trees/list.md)
- At the beginning (left side) of a row
- Indicate the status on a type icon

### Best practices

- Badge on circle status shall be colored according definition for the [status colors](../../fundamentals/colors/ui-colors.md#status-deprecated)
- Use it with severity symbols when there is a need to identify different severities
- The color of the icon should remain constant according definition in [UI colors](../../fundamentals/colors/ui-colors.md#ui-deprecated)
