...
The HTTP error page examples are making use of the [info page](../components/pages/info-page.md)
component (`si-info-page`) as the base layout.

Import `SiInfoPageComponent`, `NgOptimizedImage` for the illustration, and `SiTranslatePipe` when
its alternative text uses a translation key.
...
@Component({
  imports: [NgOptimizedImage, SiInfoPageComponent, SiTranslatePipe],
  ...
})
```

Set the heading, description, and alternative text using the keys for the relevant HTTP error.
Project the mapped illustration and suitable recovery actions into the info page.

```html
<si-info-page
  titleText="SI_HTTP_ERRORS.NOT_FOUND.HEADING"
  instructions="SI_HTTP_ERRORS.NOT_FOUND.DESCRIPTION"
>
  <img
    class="si-info-image"
    [alt]="'SI_HTTP_ERRORS.NOT_FOUND.ALT' | translate"
    ngSrc="./assets/images/magnifier.svg"
    width="260"
    height="190"
  />
  <div class="si-info-actions">
    <button type="button" class="btn btn-secondary">Go back</button>
    <button type="button" class="btn btn-secondary">Search app</button>
  </div>
</si-info-page>
```

For Siemens-internal applications, localized headings, descriptions, and alternative texts are
available through the Siemens-internal language packs. Append `.HEADING`, `.DESCRIPTION`, or `.ALT`
to one of these key prefixes:

| HTTP code | Translation key prefix                 | Illustration                                            |
| --------- | -------------------------------------- | ------------------------------------------------------- |
| `400`     | `SI_HTTP_ERRORS.BAD_REQUEST`           | [document.svg](http-error-illustrations/document.svg)   |
| `401`     | `SI_HTTP_ERRORS.UNAUTHORIZED`          | [lock.svg](http-error-illustrations/lock.svg)           |
| `404`     | `SI_HTTP_ERRORS.NOT_FOUND`             | [magnifier.svg](http-error-illustrations/magnifier.svg) |
| `418`     | `SI_HTTP_ERRORS.TEAPOT`                | [teapot.svg](http-error-illustrations/teapot.svg)       |
| `500`     | `SI_HTTP_ERRORS.INTERNAL_SERVER_ERROR` | [plug.svg](http-error-illustrations/plug.svg)           |
| `504`     | `SI_HTTP_ERRORS.GATEWAY_TIMEOUT`       | [hourglass.svg](http-error-illustrations/hourglass.svg) |

### Example

Select an HTTP status code to preview the corresponding error page and illustration.

<si-docs-component example="http-error-pages/http-error-pages" height="650"></si-docs-component>
